  ...
  for (i = 0; i < results.length; i++) {
    ...
    if (r.get("canFetchTokenDetails") === 1) {
      ...
      try {
        ...
        const priceResult = await Moralis.Web3API.token.getTokenPrice(
          priceOptions
        );
      ...
      try {
        ...
        totalSupply =
          parseInt(
            await Moralis.Web3API.native.runContractFunction(mcapOptions)
          ) /
          10 ** r.get("decimals");
      } catch (e) {
        // logger.error(`${e}`);
      ...
      try {
        if (r.get("gecko_id") !== "") {
          ...
          const url =
            ...
            "/history?date=" +
            date_string;
          const response = await Moralis.Cloud.httpRequest({ url: url }).catch(
            (e) => {
              logger.info(
                `Gecko request failed for token: ${r.get("symbol")}` + e.status
              );
            }
          );
          geckoMarketCap = JSON.parse(response.text)["market_data"][
            "market_cap"
          ]["usd"];
        }
      } catch {
